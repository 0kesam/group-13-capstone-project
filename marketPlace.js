...
(() => {
  ...
  function productFromCard(cardEl) {
    ...
    const titleEl =
      cardEl.querySelector("span") || cardEl.querySelector("h3") || null;
    ...
    return {
      id:
        ...
        `${category}-${date}-${Math.random().toString(36).slice(2, 8)}`,
        ...
        viewBtn.addEventListener("click", (e) => {
          ...
          if (product.farmerId) {
            localStorage.setItem(
              "selectedFarmer",
              JSON.stringify({ farmerId: product.farmerId })
            );
          } else {
            // If no farmerId available, store a fallback object
            localStorage.setItem(
              "selectedFarmer",
              JSON.stringify({ farmerId: product.id })
            );
          }
    ...
    function filterGrid() {
      ...
      document.querySelectorAll(".product__cards").forEach((card) => {
        ...
        const matchesCategory =
          category === "all" || cardCategory === category.toLowerCase();
