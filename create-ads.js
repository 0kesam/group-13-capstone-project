...
submitBtn.addEventListener("click", (e) => {
  ...
  const loggedInFarmer = JSON.parse(localStorage.getItem("loggedInUser"));
  if (!loggedInFarmer || !loggedInFarmer.id) {
    alert("You must be signed in as a registered farmer to post an ad.");
    return;
  }
  ...
  Promise.all(imagePromises).then((images) => {
    const adData = {
      ...
      images,
      datePosted: new Date().toISOString(),
      // ---- Attach logged-in farmer info here ----
      farmerId: loggedInFarmer.id,
      farmerName: loggedInFarmer.name,
      farmerPhone: loggedInFarmer.phone || '',
      farmerLocation: loggedInFarmer.location || ''
    };
